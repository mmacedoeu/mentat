...
class MentatTests: XCTestCase {
    ...
    func transactCitiesSchema(inProgress: InProgress) throws -> TxReport {
        let vocab = try readCitiesSchema()
        let report = try inProgress.transact(transaction: vocab)
        return report
    }

    func transactSeattleData(inProgress: InProgress) throws -> TxReport {
        let data = try readSeattleData()
        let report = try inProgress.transact(transaction: data)
        return report
    }
    ...
    func openAndInitializeCitiesStore() -> Mentat {
        guard let mentat = self.store else {
            ...
            let _ = try! self.transactCitiesSchema(inProgress: inProgress)
            let _ = try! self.transactSeattleData(inProgress: inProgress)
    ...
    func populateWithTypesSchema(mentat: Mentat) -> (TxReport?, TxReport?) {
        do {
            ...
            """
            let report = try inProgress.transact(transaction: schema)
            let stringEntid = report.entidForTempId(tempId: "s")!
            ...
            """
            let dataReport = try inProgress.transact(transaction: data)
            try inProgress.commit()
    ...
    func test2TransactEntities() {
        do {
            ...
            XCTAssertNotNil(report)
            assert(report.txId > 0)
            let entid = report.entidForTempId(tempId: "a17592186045438")
            assert(entid == 65566)
        } catch {
            assertionFailure(error.localizedDescription)
        }
    ...
    func testBindLong() {
        ...
        let aEntid = report!.entidForTempId(tempId: "a")
    ...
    func testBindRef() {
        ...
        let bEntid = report!.entidForTempId(tempId: "b")
